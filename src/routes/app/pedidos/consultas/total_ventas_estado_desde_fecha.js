...
export async function post(req, res, next) {
    ...
    Cliente.aggregate().match({ "direcciones_asociadas.estado": estado ,activo:true}).group({
        ...
        Pedido.aggregate().match(or_consulta).group({
            _id: "$cliente.nombre",
            count: { $sum: 1 },
            total_ventas_cliente: { $sum: "$total_pedido" },
            lista_pedidos: { $push: { id: "$_id", cliente: "$cliente.nombre", fecha: "$fecha",total_pedido:"$total_pedido" } }
        }).sort({"total_ventas_cliente":-1}) .then((pedidos_en_estado)=>{
            res.send({ ok: true, clientes_en_estado,pedidos_en_estado});
        })
