<script>
  import Archivo from "./Archivo_editar_producto.svelte";
  ...
  function handleChange2(e) {
   ...
   //console.log(i);
    ...
    fr.onload = () => {
      ...
      img.src = fr.result;
      //document.body.appendChild(img);
      $lista_archivos_uploads_edicion.push({
        name: file.name,
        size: file.size,
        base64: img.src
      });
      $lista_archivos_uploads_edicion = $lista_archivos_uploads_edicion;
...
<table>
  ...
  <tbody>
    <tr>
      {#each $lista_archivos_uploads_edicion as file, i}
        <!-- <tr>
        <td>{i})</td>
        <td>{file.name}</td>
        <td>{file.size}</td>
