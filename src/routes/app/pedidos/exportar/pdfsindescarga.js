...
export async function get(req, res, next) {
  ...
  if (!origen) return res.send('Consulta no se pudo realizar')
  if (origen != "pedidos1" && origen != "pedidos2" && origen != "pedidos3") return res.send('Consulta no se pudo realizar')
  ...
  fs.readFile('src/routes/app/pedidos/exportar/_testsindescarga.html', 'utf8', (err, data) => {
    if (err) {
      console.log(err)
      res.send("error ");
      return;
...
async function row_en_texto(registro, i, tipo_cambio) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      <div class="descripcion">${registro.producto.nombre} ${tipo_cambio} </div>
...
async function folios_productos(folios) {
  var texto = ""
  if (folios.length == 0 || !folios) {
    return "";
  }
  texto = `<br>Folios:<div class="folios">`;
  for (let i = 0; i < folios.length; i++) {
    const element = folios[i];
    texto += element + ", ";
    //console.log(element)
