<script>
  ...
  let marcas = []; //marcas para el select
  let marcaSelect = "todas";
      ...
      .then((respuesta) => {
        ...
        if (respuesta.ok) {
          ...
          if (ordenado) {
            ...
            intervalo =
              respuesta.children[respuesta.children.length - 1]
                .unidades_vendidas /
              (intervalos_horizontales - 1);
          }
  ...
  function ObtenerMarcasSelect() {
    postData("app/pedidos/consultas/selectMarcas", {}).then((respuesta) => {
      if (respuesta.ok) {
        console.log(respuesta);
  ...
  <div>
    ...
    {:else if existen_resultados_a_mostrar}
      ...
      <h2 style="color:white;padding-top:10px;">
        {ordenado
          ? "Productos de más a menos vendidos"
          : "Productos de menos a más vendidos"}
      </h2>
          ...
          <g class="bars">
            {#each points as point, i}
              <rect
                ...
                height={height -
                  padding.bottom -
                  yScale(point.unidades_vendidas)}
              />
            {/each}
          </g>
        </svg>
