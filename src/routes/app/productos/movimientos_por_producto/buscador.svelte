<script>
import {ventas_producto_selecto} from './../../../stores';
...
function derecha() {
    if(pagina_actual==$ventas_producto_selecto.paginas)return;
    pagina_actual ++;
    marcar_como_pendiente();
}
...
<table>
    <tr>
      <td>
        <button on:click={izquierda} class:desactivado={pagina_actual ==1} class="pointer boton" disabled={pagina_actual ==1}>
      ...
      <td title="Cuenta de logs" class="paginas centro_regular izquierda">{$ventas_producto_selecto.paginas}</td>

      <td>
        <button on:click={derecha} class:desactivado={pagina_actual >= $ventas_producto_selecto.paginas} disabled={pagina_actual >= $ventas_producto_selecto.paginas} class="pointer boton">
          <i class="material-icons vertical-alineado">chevron_right</i>
        </button>
      </td>
      <td class="coincidencias " title="Coincidencias">
      coincidencias:
      {$ventas_producto_selecto.coincidencias} 
      </td>
      <td>
