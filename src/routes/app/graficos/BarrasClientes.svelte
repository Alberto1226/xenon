<script>
  ...
  function obtenerPancake() {
    ...
    postData("app/pedidos/consultas/pancake_barras_clientes", {
      desde,
      hasta,
      pagina_actual,
  ...
  function obtenerListaSinComsumir() {
    ...
    consulta_no_vendidos_fue_realizada = false;
    // resultado_ventas.lista_pedidos = [];
    postData("app/pedidos/consultas/pancake_barras_clientes_sin_compra", {
      desde,
      hasta,
      pagina_actual,
      step: cantidad_por_step,
      ...
      .then((respuesta) => {
        ...
        //  Checar si se logro guardar el cliente
        if (respuesta.ok) {
          lista_productos_sin_venta = respuesta.clientesSinCompras;
        } else {
          http_prods_sin_venta_ocupado = false;
          $mensajes_app.push({
            tipo: "error",
  ...
  let select = (node) => {
    ...
    resumen_producto = `
        <div class="centrado titulo_formulario" style="color: black !important;">${producto.nombre_producto}</div>
        ...
        <hr>
        <div class="indice_row centrado">  Costo de Compras</div>
...
<div class="contenedor" in:fade={{ duration: 400 }}>
  <div>
    <div class="centrado" style="background: rgb(79, 146, 151);">
      <table style=" padding-top: 5px; width: 100%; padding-right: 20px;">
        <tr>
          ...
          <td class="titulo_formulario" style="color: black !important;">
            Clientes Con Compras entre fechas
          </td>
          ...
          <td>
            ...
            <Button
              ...
              title={http_prods_sin_venta_ocupado
                ? "ocupado..."
                : "Obtener productos activos que no se vendieron en el periodo"}
            >
    ...
    {:else if existen_resultados_a_mostrar}
      ...
      <h2 style="color:white;padding-top:10px;">
        {ordenado
          ? "Clientes de más a menos costo de compras"
          : "Clientes de menos a más costo de compras"}
      </h2>
            ...
            {#each points as point, i}
              <g class="tick" transform="translate({xScale(i)},{height + 10})">
                <text x={barWidth / 2} y="-40">
                  {width > 380
                    ? point.nombre_producto
                    : formatMobile(point.nombre_producto)}
                </text>
                ...
                <text class="unidades_vendidas" x={barWidth / 2} y="-20">
                  Compras {borrar_ultimos_2(
                    formato_precio(point.unidades_vendidas),
                  )}
                </text>
              </g>
            {/each}
          ...
          <g class="bars">
            {#each points as point, i}
              <rect
                ...
                height={height -
                  padding.bottom -
                  yScale(point.total_ventas_periodo)}
              />
            {/each}
          </g>
        </svg>
      ...
      {#if consulta_no_vendidos_fue_realizada}
        <div class="centrado">
          <h3 style="color:white">
            ...
            Clientes
            <b>activos</b>
            sin compras en el periodo (total {lista_productos_sin_venta.length})
          </h3>
            ...
            {#each lista_productos_sin_venta as prod_no_vendido, i}
              ...
              <table class="row">
                <tr>
                  ...
                  <td style="width:150px;">{prod_no_vendido.correo}</td>
                </tr>
              </table>
            {:else}
              <!-- empty list -->
