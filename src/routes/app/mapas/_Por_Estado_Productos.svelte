<script>
  ...
  import Lista_de_productos from "./_componentes/Lista_de_productos.svelte";
  ...
  function total_ventas_estado() {
    ...
    postData("app/pedidos/consultas/total_ventas_producto_estado", {
      estado: estado_tmp,
      periodicidad
    })
      .then(respuesta => {
       //console.log(respuesta);
       //console.log(respuesta.clientes_en_estado.lista_clientes);
        ...
        if (respuesta.ok) {
          if (respuesta.ok) {
            if(respuesta.clientes_en_estado[0].lista_clientes.length>0){
              clientes_en_estado.lista_de_clientes_en_estado = respuesta.clientes_en_estado[0].lista_clientes;
              //ordenar clientes por nombre
               clientes_en_estado.lista_de_clientes_en_estado.sort((a, b) =>
              a.nombre > b.nombre ? 1 : -1
            );
              productos_por_estado = respuesta.productos_por_estado;
            }
          }
    ...
    <tr>
      ...
      <td>
        <div class="centrado no_select" style="padding-top: 10px;">
          <h3>Analiza Productos por estado</h3>
          <br />
    ...
    <table>
      <tr>
        ...
        <td>
          <Lista_de_productos
            bind:ubicacion={estado}
            bind:lista={productos_por_estado} />
        </td>
