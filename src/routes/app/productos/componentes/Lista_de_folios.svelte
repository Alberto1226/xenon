<script>
    ...
    function borrar() {
        ...
        ocupado = true;
        confirmaciones = 0;
        let folios_a_enviar = selectos.map((elem) => {
            return elem.folio;
        });
        postData("app/productos/borrar_folios", {
            producto_id: producto._id,
            folios: folios_a_enviar,
        }).then((resultado) => {
            ...
            if ((resultado.ok = true)) {
                cuantos_selectos();
                mensaje_bueno("Se borraron los folios selectos");
            } else {
                mensaje_error("Algo salio mal, intentalo más tarde.");
            }
        });
    }

    function borrar_local() {
        let folios_a_borrar = selectos.map((elem) => {
            return elem.folio;
        });
        let lista_filtrada = folios.filter(
            (elem) => !folios_a_borrar.includes(elem.folio)
        );
        folios = lista_filtrada;
        ...
        <div class="flex">
            ...
            {#if guardando_folios == false}
                {#if $usuario_db.rol == "administrador"}
                    {#if confirmaciones == 0 && cantidad_selecta > 0}
                        <Button
                            color="red"
                            raised
                            dense
                            on:click={() => (confirmaciones = 1)}
                        >
                    ...
                    {:else if confirmaciones == 1}
                        <Button
                            ...
                            raised
                            dense
                            on:click={() => (confirmaciones = 2)}
                        >
                            <i class="material-icons">delete</i> confirma 2 veces
                            más
                        </Button>
                    {:else if confirmaciones == 2}
                        <Button color="red" raised dense on:click={borrar}>
                            <i class="material-icons">delete</i> confirma 1 vez más
                        </Button>
                    {/if}
                {/if}
            {:else}
