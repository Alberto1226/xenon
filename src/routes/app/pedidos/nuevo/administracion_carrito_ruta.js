...
import { devolver_producto_db } from '../editar/_server_cambiar_cantidad/devolver_producto_db';
import { devolver_carrito_db } from '../editar/_server_cambiar_cantidad/devolver_carrito_db';
import * as accesos from "../../accesos";
import { snap_por_cambio_en_pedido } from '../editar/_producto_snaplogs/snap_por_cambio_en_pedido';
import { devolver_prod_snap_log_fixBug } from '../editar/_server_cambiar_cantidad/devolver_prod_snap_log_fixBug';
...
export async function post(req, res, next) {
    ...
    if (donde === "consultaRutas") {
        // console.log("consultaRutas");
    ...
    if (masData) {
        ...
        // Consultar si ya existe una salida con este carrito_id
        const salidaExistente = await SalidasVentas.findOne({ id_carritos: id });
        if (!salidaExistente) {
            await crearSalidaRuta(id, masData, req);
        }
        // return res.send({ ok: true, mensaje: "Salida creada", salida: nuevaSalida });
    }
    ...
    // console.log('son_suficientes', son_suficientes);
...
async function crearSalidaRuta(id, masData, req) {
    // Consultar el carrito para obtener el folio
    const carrito = await Carrito.findById(id);
    if (!carrito) {
        return res.send({ ok: false, mensaje: "No se encontró el carrito" });
    }

    // Generar el folio de salida
    const folioSalida = await generarFolioSalida(carrito.folio);
    console.log("Folio de salida generado:", folioSalida);
    console.log("Fecha estimada:", masData.fecha_estimada);

    //agregar el folioSalida en carrito
    await Carrito.findByIdAndUpdate(id, { folio_salida: folioSalida }, { new: true });

    // Crear la salida
    const nuevaSalida = new SalidasVentas({
        id_carritos: id,
        id_usuario: req.user._id,
        folio_salida: folioSalida,
        fecha_salina: masData && masData.fecha_estimada ? masData.fecha_estimada : new Date(),
...
// Función para generar el folio de salida
