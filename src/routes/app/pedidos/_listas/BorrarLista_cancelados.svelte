<script>
  ...
  import { ui, pedidos_cancelados, postData } from "./../../../stores";
  import Nuevo from "./../nuevo/nuevo.svelte";
  import Row from "./Row_cancelados.svelte";
  ...
  onMount(() => {
    //console.log($pedidos_cancelados.lista);
    lista = $pedidos_cancelados.lista;
  });
  ...
  $: total_paginas = Math.floor(
    $pedidos_cancelados.lista.length / limite_lista
  );
  ...
  function obtener_pedidos(params) {
    http_ocupado = false;
    postData("app/pedidos/lista_de_pedidos_cancelados", {
      buscando,
      pagina_actual
    })
      .then(res => {
        ...
        if (res.ok) {
          //console.lo(res);
          $pedidos_cancelados.lista = res.lista;
          $pedidos_cancelados.lista_actualizada = new Date(); //  cuando se actualizo la lista completa por ultima vez
          $pedidos_cancelados = $pedidos_cancelados;
          lista = $pedidos_cancelados.lista;
          $pedidos_cancelados.total_registros = res.numero_total;
