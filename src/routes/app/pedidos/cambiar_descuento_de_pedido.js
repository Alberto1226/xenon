...
export async function post(req, res, next) {
    ...
    accesos.logActividad('pedidos/cambiar_descuento_de_pedido', req.user, { descuentoAnterior: descuentoOriginal,
         descuentoNuevo: descuento_nuevo, 
         resumenOriginal: historial_precios_originales, 
         resumenPrevio, resumenPosterior, 
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            for (let i = 0; i < lista.length; i++) {
                ...
                //  PROMO es ?
                var es_promo = false ;
                if(registro.promo !=undefined){
                    es_promo = registro.promo.con_promo;                        
                }
                console.log({
                    nombre:registro.producto.nombre,
                    promo :registro.promo
                });
                historial_precios_originales.push({ 
                    id: producto_orig.producto._id, 
                    nombre: producto_orig.producto.nombre,
                     precio: producto_orig.producto.precio,
                     es_promo, 
                     aplica_descuento: producto_orig.producto.aplicar_descuento_distribuidor 
                    })
                ...
                if (producto_orig.producto.aplicar_descuento_distribuidor == true) {
                    ...
                    if(es_promo ==false){
                        ...
                        //     SI ES PROMO,  Garantizar que el precio no se afecte con el precio original del producto al final de esta fn
                        nuevo_precio = registro.producto.precio;
                    }
                }
                else {
                        //      Garantizar que el precio no se afecte con el precio original del producto al final de esta fn
                        nuevo_precio = registro.producto.precio;
                }
...
async function descuento_aplicado_correctamente(lista_original, lista_nueva, descuento_nuevo) {
    ...
    for (let i = 0; i < lista_original.length; i++) {
       ...
       // console.log(element);
        //  PROMO es ?
        var es_promo = false ;
            es_promo = element.es_promo?element.es_promo:false;                        
        
        const precio_correcto = element.precio - (element.precio * descuento_nuevo / 100);
        var exito = precio_correcto === lista_nueva[i].precio;
       // console.log(i+"----");
       // console.log({aplica_des:element.aplica_descuento === false , es_promo});
       // console.log(i+"----");
        if(element.aplica_descuento === false || es_promo ==true){
            //exito = element.precio === lista_nueva[i].precio;
            //      siempre sera true por que no cambia del precio que viene en el pedido nunca
            //  para promo como para los que no se aplica descuento
            //  no se debe de tocar
            exito =true;
        }
       // console.log({exito});
        if (exito==true) contador_exito++;
      //  console.log({contador_base});
       // console.log({contador_exito});
        if (i + 1 === lista_original.length) {
       //     console.log({contador_base});
       //     console.log({contador_exito});
       //     console.log("Finaliza-------descuento_aplicado_correctamente ");
